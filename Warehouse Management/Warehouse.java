...
public class Warehouse {
    ...
    private void addToEnd(int id, String name, int stock, int day, int demand) {
        ...
        // use .add??
        Product t = new Product(id, name, stock, day, demand);
        sectors[id % 10].add(t);
    }
    ...
    public void restockProduct(int id, int amount) {
        ...
        for (int i = 1; i <= sectors[i % 10].getSize(); i++) {
            // Product restocked_product = sector.get(i);
            if (id == sectors[id % 10].get(i).getId()) {
                // .updatestock????
    ...
    public void deleteProduct(int id) {
        ...
        // int size_of_sector = sector.getSize();

        for (int i = 1; i <= sectors[id % 10].getSize(); i++) {
            // Product sectors[id%10].get(i)=sector.get(i);
            if (id == sectors[id % 10].get(i).getId()) {
    ...
    public void purchaseProduct(int id, int day, int amount) {
        ...
        for (int i = sectors[id % 10].getSize(); i > 0; i--) {
            // Product incoming_product = sectors[i%10].get(i);
            if (id == sectors[id % 10].get(i).getId() && amount <= sectors[id % 10].get(i).getStock()) {
    ...
    public void betterAddProduct(int id, String name, int stock, int day, int demand) {
        // IMPLEMENT THIS METHOD
        /*
         * int sector_number = id % 10;
         * Sector sector = sectors[sector_number];
         * if (sector.getSize() < 5) {
         * addToEnd(id, name, stock, day, demand);
         * fixHeap(id);
         * return;
         ...
         * int checker = 1 + sector_number;
         * while (!complete_search) {
         * if (checker == 10) {
         * checker = 0;
         * }
         * sector = sectors[checker];
         * if (checker == sector_number) {
         * evictIfNeeded(id);
         * addToEnd(id, name, stock, day, demand);
         * fixHeap(id);
         ...
         * return;
         * }
         * int capacity = sector.getSize();
         * if (capacity < 5) {
         * Product t_new = new Product(id, name, stock, day, demand);
         * sector.add(t_new);
