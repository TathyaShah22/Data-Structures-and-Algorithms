...
public class PredictThanosSnap {
    ...
    public static void main(String[] args) {
        ...
        long given_seed = StdIn.readLong();
        StdOut.setFile(outputFile);
        StdRandom.setSeed(given_seed);

        int nodes_from_file = StdIn.readInt();
        int n = nodes_from_file;
        int[][] next_node = new int[nodes_from_file][nodes_from_file];

        for (int i = 0; i < nodes_from_file; i++) {
            for (int j = 0; j < nodes_from_file; j++) {
                next_node[i][j] = StdIn.readInt();
                /*
                * int next_to_node = 0;
                 * int temp;
                 * temp = next_to_node;
        ...
        for (int i = 0; i < nodes_from_file; i++) {
            for (int j = 0; j < nodes_from_file; j++) {
                ...
                // next_node[i][j] = 0;
                // a[j]=0;
                //System.out.print((next_node[i][j] + " "));
            }
            //System.out.println();
        }
        int edges = 0;
