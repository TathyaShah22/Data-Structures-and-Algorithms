...
public class Board {
    ...
    public void addRandomTile() {
        ...
        // [boardspot, boardspot, boardspot, boardspot, boardspot]
            // 0           1          2          3          4
            // where 0 is (i,j) 1 is (i1,j1)  2 is (2i,2j) ...
            
        // pick a random tile
        int v = StdRandom.uniform(0,openSpaces.size());
        //get a random value for tile
        
        int random_value;
        if(StdRandom.uniform(0.0,1.0)<0.1){
            random_value=4;
        }
            else{
                random_value=2;
            }
        //set random value to random tile.
           
            int rows = openSpaces.get(v).getRow();
            int columns = openSpaces.get(v).getCol();
            gameBoard[rows][columns] =  random_value;
    ...
    public void swipeLeft() {
        ...
        //2 for loops to traverse through the arrays

        for (int i = 0; i < gameBoard.length; i++) {
            int counter = 0;
            for (int j = 0; j < gameBoard[i].length; j++) {
                //if the [i][j] is not 0, that means there a number present there
                //when it is not 0, [i][counter++] must equal [i][j] of gameboard
                // this means that any space that is not 0 will be recorded 
                if (gameBoard[i][j]!= 0) {
                    gameBoard[i][counter] = gameBoard[i][j];
                    counter++;
                }
            }
            //for (int j = 0; j < gameBoard.length.getRow(); j++) {
                //for (int j = 0; j < gameBoard.length.getCol(); j++) {
                    
                //}
                //moves the 2/4 to a zero value
    ...
    public void mergeLeft() {
        ...
        //[ 0 2 0 2 ]  [ 2 2 0 0 ] [ 4 0 0 0 ]
        for (int i = 0; i < gameBoard.length; i++) {
            int j=1;
            while (j< gameBoard[i].length) {
            if(gameBoard[i][j]==gameBoard[i][j-1]){
                gameBoard[i][j-1]=2*gameBoard[i][j-1];
                gameBoard[i][j]=0;
                //gameBoard[i][j-1]=0;

            }
                //if (gameBoard[i]==gameBoard[i+1]) {
                    //gameBoard[i][i+1]=2*gameBoard[i][i+1];
                    //gameBoard[i+1]=2*gameBoard[i+1];
                    //gameBoard[i][i+1]=0;
                j++;
            }
        }
    ...
    public void rotateBoard() {
        transpose();
        flipRows();
         //need to finish transpose & fliprows function before doing this 
    }
    ...
    public void transpose() {
        ...
        //make temp variable to swap / transpose the rows and columns 
        int use_variable=0;
        for (int i = 0; i < gameBoard.length; i++) {
            for (int j = 0; j < i; j++) {
                use_variable=gameBoard[i][j];
                gameBoard[i][j]=gameBoard[j][i];
                gameBoard[j][i]=use_variable;
            }
        }
    }
    ...
    public void flipRows() {
        // WRITE YOUR CODE HERE
        int[][] new_result = new int[4][4];//new array to store the vlaues 
        int temp=0;
        for (int i = 0; i < gameBoard.length; i++) {
            for (int j = gameBoard[0].length-1; j >= 0; j--) {
                new_result[i][temp]=gameBoard[i][j];
                temp++;
            }
            //start temp from 0 again to go through all columns in the row
            temp=0;
        }
        //int[][] gameBoard = int[][] new_result;
        gameBoard=new_result;
    }
    ...
    public void makeMove(char letter) {
        ...
        //need to rotate board in ways that u can use swipe left and merge left 
        if (letter =='U') {
            rotateBoard();
            rotateBoard();
            rotateBoard();
